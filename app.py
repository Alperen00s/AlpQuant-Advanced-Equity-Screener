...
def generate_pdf_file(hisse, veri_dict, fig):
    ...
    pdf.set_text_color(0, 0, 0)
    pdf.set_font("Arial", 'B', 14)
    pdf.cell(0, 10, "Appendix: Glossary of Terms", ln=1)
    pdf.line(10, pdf.get_y(), 200, pdf.get_y())
    pdf.ln(5)
    
    explanations = {
        "Price": "Current market price of the asset.",
        "P/E": "Price-to-Earnings: Lower value may indicate undervaluation compared to peers.",
        "P/B": "Price-to-Book: Compares market cap to book value of the company.",
        "PEG": "P/E to Growth: Ratio < 1 indicates potential undervaluation given its growth.",
        "ROE%": "Return on Equity: Efficiency in generating profits from net assets.",
        "Div Yield%": "Dividend Yield: Annual dividend payment relative to stock price.",
        "Beta": "Systemic Risk: > 1.0 means more volatile (aggressive) than the market index.",
        "RSI": "Relative Strength Index: Momentum indicator (>70 Overbought, <30 Oversold).",
        "BBW%": "Bollinger Bands Width: Indicates price volatility and potential price squeeze.",
        "MACD": "Trend-following momentum indicator (Bull = Short-term upward momentum).",
        "Trend": "50-Day vs 200-Day Simple Moving Average positioning (Golden Cross)."
    }
    
    for k, expl in explanations.items():
        pdf.set_font("Arial", 'B', 10)
        pdf.set_text_color(60, 60, 60)
        pdf.cell(25, 6, tr2eng(f"{k}:"), ln=0)
        pdf.set_font("Arial", '', 10)
        pdf.set_text_color(100, 100, 100)
        pdf.cell(0, 6, tr2eng(expl), ln=1)
    
    pdf.ln(10)
    pdf.set_y(-25)
    pdf.set_font("Arial", 'I', 8)
    pdf.set_text_color(150, 150, 150)
    pdf.cell(0, 10, "Report automatically generated by AlpQuant v3.0. Not financial advice.", align='C')
